"""
Milestone 4.3D (Financial Health Score) / Adım 8: `app/engines/health_
...
from app.engines.health_score.service import apply_hard_fail_rules
...
def test_negative_equity_triggers_ceiling_25():
...
def test_severe_debt_service_shortfall_triggers_ceiling_35():
...
def test_both_rules_trigger_simultaneously_lowest_ceiling_wins():
    ...
    assert score == Decimal("25")  # min(90, 25, 35) = 25
...
def test_preliminary_score_already_below_ceiling_stays_unchanged_but_still_reports_trigger():
    ...
    assert score == Decimal("10")  # min(10, 25) = 10
    assert triggered == ("NEGATIVE_EQUITY",)
