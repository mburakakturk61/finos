"""
Milestone 4.3E (Credit Score Engine) / Adım 4: `app/engines/credit_
...
`collect_ratio_signals`/`compute_ratio_score`, Health Score'dan DOĞRUDAN
yeniden kullanılan (kopyalanmayan) fonksiyonlardır -- bu testler o
yeniden kullanımın Credit'in KENDİ 6-kategorili taksonomisiyle DOĞRU
şekilde birleştiğini kanıtlar.
"""
...
from app.engines.health_score.service import collect_ratio_signals, compute_ratio_score
...
def test_full_coverage_category_has_coverage_ratio_one():
    ratio_result, benchmark_result = _real_result_jsons(with_prior_period=True)
    signals = collect_ratio_signals(ratio_result, benchmark_result)
    scored, excluded = filter_scoreable_signals(signals, "debt_service_capacity")
    weights = registry_ratio_weights_for_category("debt_service_capacity")

    breakdown = build_category_breakdown("debt_service_capacity", scored, excluded, weights)
    assert breakdown.coverage_ratio == Decimal("1")


def test_excluded_duplicate_ratios_appear_in_contributions_with_zero_weight_and_explainability_role():
    ratio_result, benchmark_result = _real_result_jsons(with_prior_period=True)
    signals = collect_ratio_signals(ratio_result, benchmark_result)
    ...
    assert wc_contribution.role == "explainability_only"


def test_scored_critical_ratio_contribution_has_nonzero_weight_score_and_critical_role():
    ratio_result, benchmark_result = _real_result_jsons(with_prior_period=True)
    signals = collect_ratio_signals(ratio_result, benchmark_result)
    scored, excluded = filter_scoreable_signals(signals, "liquidity")
    weights = registry_ratio_weights_for_category("liquidity")
    ...
    assert cr_contribution.role == "critical"


def test_scored_supporting_ratio_contribution_has_supporting_role():
    ratio_result, benchmark_result = _real_result_jsons(with_prior_period=True)
    signals = collect_ratio_signals(ratio_result, benchmark_result)
    scored, excluded = filter_scoreable_signals(signals, "liquidity")
    weights = registry_ratio_weights_for_category("liquidity")
    ...
    breakdown = build_category_breakdown("liquidity", scored, excluded, weights)
    qr_contribution = next(
        c for c in breakdown.ratio_contributions if c.ratio_code == "quick_ratio"
    )
    assert qr_contribution.role == "supporting"


def test_raw_score_is_clamped_between_0_and_100_for_all_active_categories():
    ratio_result, benchmark_result = _real_result_jsons(with_prior_period=True)
    signals = collect_ratio_signals(ratio_result, benchmark_result)
...
def test_redistributed_weights_field_matches_input_weights_dict():
    ...
    weights = registry_ratio_weights_for_category("activity")

    breakdown = build_category_breakdown("activity", scored, excluded, weights)
...
def test_registry_ratio_weights_for_category_sums_to_one_for_all_active_categories():
    for category in ACTIVE_CATEGORIES:
        weights = registry_ratio_weights_for_category(category)
        assert sum(weights.values()) == Decimal("1"), category


def test_active_categories_matches_credit_score_own_six_category_taxonomy():
    assert set(ACTIVE_CATEGORIES) == {
        "liquidity", "leverage", "debt_service_capacity", "profitability", "activity", "growth",
    }
    assert "efficiency" not in ACTIVE_CATEGORIES  # Health Score'unkiyle birlesti
    assert "cash_flow" not in ACTIVE_CATEGORIES  # yapisal olarak %0
