"""
Milestone 4.3D (Financial Health Score) / Adım 6: `app/engines/health_
score/service.py`'nin coverage normalization fonksiyonları (`redistribute_
category_weights` / `compute_overall_data_coverage_ratio` / `determine_
insufficient_data_and_warning` / `resolve_cross_category_weights`) için
...
from app.engines.common.health_score_types import CategoryBreakdown, CategoryWeightProfile
...
def test_resolve_cross_category_weights_all_present_matches_global_profile():
    profile = CategoryWeightProfile(
        ...
        category_weights={
            ...
            "profitability": Decimal("0.20"), "activity": Decimal("0.15"),
            "efficiency": Decimal("0.10"), "growth": Decimal("0.10"),
        },
    )
