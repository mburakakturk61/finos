"""
Milestone 4.3D (Financial Health Score) / Adım 9: `app/engines/health_
score/service.py::compute_confidence_score` / `weighted_reliabilities_
for_category` (Bölüm 10) için gerçek, çalıştırılabilir birim testleri.
"""
...
def test_all_medium_reliability_is_also_capped_since_above_ceiling():
    # medium=0.66 ham degeri, tavan 0.60'in UZERINDE -> tavana cekilir.
    result = compute_confidence_score([("medium", Decimal("1"))])
    assert result == Decimal("0.60")


def test_all_medium_low_reliability_stays_below_ceiling_uncapped():
    result = compute_confidence_score([("medium_low", Decimal("1"))])
    assert result == Decimal("0.5")


def test_mixed_weighted_reliabilities_computed_correctly():
    # 0.5 agirlik high(1.0) + 0.5 agirlik medium_low(0.5) -> ham=0.75 -> tavan 0.60
...
def _contribution(ratio_code, reliability, weight_applied, *, score=Decimal("70"), excluded=False):
    return RatioContribution(
        ...
        ratio_weight_applied=weight_applied, tier_fallback_used=False,
...
def test_weighted_reliabilities_skips_excluded_and_non_evaluated():
    breakdown = CategoryBreakdown(
        ...
        ratio_contributions=(
            _contribution("current_ratio", "high", Decimal("0.5")),
            _contribution("quick_ratio", "medium", Decimal("0.5")),
            _contribution("working_capital_ratio", "high", Decimal("0"), excluded=True),
            RatioContribution(
                ...
                tier_fallback_used=False, reliability="not_calculable", is_duplicate_excluded=False,
            ),
        ),
    )
...
def test_weighted_reliabilities_applies_category_weight_multiplier():
    breakdown = CategoryBreakdown(
        ...
        redistributed_weights={}, critical_overrides_applied=(),
        ratio_contributions=(_contribution("equity_ratio", "high", Decimal("1")),),
    )
